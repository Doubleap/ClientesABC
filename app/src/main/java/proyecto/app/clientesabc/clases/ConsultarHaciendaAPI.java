...
public class ConsultarHaciendaAPI extends AsyncTask<Void,String,JsonObject> {
    ...
    AlertDialog dialog;
    public ConsultarHaciendaAPI(WeakReference<Context> c, WeakReference<Activity> a, String sociedad, String cedula){
        this.context = c;
        this.activity = a;
    ...
    protected JsonObject doInBackground(Void... voids) {
        ...
        if(mensaje.equals("")) {
            ...
            Call<ResponseBody> call = apiService.ConsultarHacienda(PreferenceManager.getDefaultSharedPreferences(context.get()).getString("CONFIG_SOCIEDAD",VariablesGlobales.getSociedad()), cedula);
            ...
            try {
                ...
                if (!response.body().contentType().toString().equals("text/html")) {
                    ...
                    respuesta = com.google.gson.JsonParser
    ...
    protected void onPreExecute() {
        ...
        if (activity.get().getWindow() == null || activity.get().getWindow().getDecorView().getWindowToken() == null)
            return;
    ...
    protected void onPostExecute(JsonObject respuesta) {
        ...
        if(xceptionFlag){
            ...
            if (act instanceof SolicitudActivity) {
                // Call method specific to SolicitudModificacionActivity
                ((SolicitudActivity) act).ActualizarRespuestaHacienda(context.get(), act, respuesta);
            }
            if (act instanceof SolicitudModificacionActivity) {
                // Call method specific to SolicitudModificacionActivity
                //((SolicitudModificacionActivity) act).ActualizarRespuestaHacienda(context.get(), act, respuesta);
            }
        }
    }
