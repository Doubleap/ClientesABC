...
public class ObtenerAutenticacionTokenAPISinc {
    ...
    AlertDialog dialog;
    public ObtenerAutenticacionTokenAPISinc(WeakReference<Context> c, WeakReference<Activity> a, String sociedad, String username, String password){
        this.context = c;
        this.activity = a;
    ...
    protected ArrayList<JsonArray> ObtenerToken(Void... voids) {
        ...
        if(mensaje.equals("")) {
            ...
            try {
                ...
                if (response != null && response.body() != null && !response.body().contentType().toString().equals("text/html")) {
                    ...
                    if(fileSize == -1){
                        //fileSize = response.body().string().length();
                    }
                    ...
                    //messageFlag = "status:'"+response.raw().message()+".'"/*+ response.raw().request().url()*/;
                    messageFlag += error;
                    xceptionFlag = true;
                }
            } catch (IOException e) {
