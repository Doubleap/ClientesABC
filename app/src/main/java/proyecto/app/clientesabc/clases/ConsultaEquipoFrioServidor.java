...
public class ConsultaEquipoFrioServidor extends AsyncTask<Void,String,ArrayList<JsonArray>> {
    ...
    private String codigoEquipoFrio;
    ...
    AlertDialog dialog;
    public ConsultaEquipoFrioServidor(WeakReference<Context> c, WeakReference<Activity> a, String codigoEquipoFrio){
        this.context = c;
        this.activity = a;
        this.codigoEquipoFrio = codigoEquipoFrio;
    }
    ...
    protected ArrayList<JsonArray> doInBackground(Void... voids) {
        ...
        try {
            ...
            if(mensaje.equals("")) {
                ...
                dos.writeUTF("ConsultaEquipoFrio");
                dos.flush();

                dos.writeUTF(String.format("%10s", String.valueOf(codigoEquipoFrio)).replace(' ', '0'));
                dos.flush();
                ...
                long s = dis.readLong();
                if (s < 0) {
                    publishProgress("Error en Consulta Equipo Frio en SAP...");
                    s = dis.readLong();
    ...
    protected void onPostExecute(ArrayList<JsonArray> estructuras) {
        ...
        if(xceptionFlag){
            //Toasty.error(context.get(),"No se pudo consultar el equipo frio: "+messageFlag,Toast.LENGTH_LONG).show();
            //activity.get().finish();
            BaseInstaladaActivity.EquipoFrioNOEncontradoenSAP(context.get(), activity.get(), estructuras);
        }
        BaseInstaladaActivity.EquipoFrioEncontradoenSAP(context.get(), activity.get(), estructuras);
    }
