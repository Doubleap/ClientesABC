...
public class DevolverPreSolicitudAPI extends AsyncTask<Void,String,ArrayList<JsonArray>> {
    ...
    AlertDialog dialog;
    public DevolverPreSolicitudAPI(WeakReference<Context> c, WeakReference<Activity> a, String bukrs, String idform, String estado, String comentario){
        this.context = c;
        this.activity = a;
        this.bukrs = bukrs;
        this.idform = idform;
    ...
    protected ArrayList<JsonArray> doInBackground(Void... voids) {
        ...
        if(mensaje.equals("")) {
            ...
            Call<ResponseBody> call = apiService.DevolverPreSolicitud(PreferenceManager.getDefaultSharedPreferences(context.get()).getString("CONFIG_SOCIEDAD",VariablesGlobales.getSociedad()), PreferenceManager.getDefaultSharedPreferences(context.get()).getString("W_CTE_RUTAHH", ""), idform, estado, comentario);
    ...
    protected void onPostExecute(ArrayList<JsonArray> mensajes) {
        ...
        if(xceptionFlag){
            ...
            try {
                ...
                //Rechazar solicitud y actualizar el formulario en la app sqlite
                SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());
                Date date = new Date();
                ...
                long modifico = mDb.delete("FormHvKof_solicitud","idform = ?", new String[]{idform});
                mDb.close();
                Toasty.success(context.get(), mensaje, Toasty.LENGTH_SHORT).show();
                if(!activity.get().isFinishing()) {
