...
public class TraerEquipoDisponibleServidor extends AsyncTask<Void,String,ArrayList<JsonArray>> {
    ...
    AlertDialog dialog;
    public TraerEquipoDisponibleServidor(WeakReference<Context> c, WeakReference<Activity> a, String centroSuministro, String tipoFormulario, String numeroPuertas){
        this.context = c;
        this.activity = a;
    ...
    protected ArrayList<JsonArray> doInBackground(Void... voids) {
        ...
        try {
            ...
            if(mensaje.equals("")) {
                ...
                dos.writeUTF("TraerEquipoDisponible");
                dos.flush();

                dos.writeUTF(PreferenceManager.getDefaultSharedPreferences(context.get()).getString("W_CTE_VWERK", ""));
                dos.flush();

                dos.writeUTF(tipoFormulario);
                dos.flush();

                dos.writeUTF(numeroPuertas);
                dos.flush();
