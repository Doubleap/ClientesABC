...
public class DesactivarRegistroCensoAPI extends AsyncTask<Void,String,ArrayList<JsonArray>> {
    ...
    AlertDialog dialog;
    public DesactivarRegistroCensoAPI(WeakReference<Context> c, WeakReference<Activity> a, String bukrs, String id){
        this.context = c;
        this.activity = a;
        this.bukrs = bukrs;
        this.id = id;
        db = new DataBaseHelper(context.get());
    ...
    protected ArrayList<JsonArray> doInBackground(Void... voids) {
        ...
        if(mensaje.equals("")) {
            ...
            Call<ResponseBody> call = apiService.DesactivarRegistroCenso(PreferenceManager.getDefaultSharedPreferences(context.get()).getString("CONFIG_SOCIEDAD",VariablesGlobales.getSociedad()), PreferenceManager.getDefaultSharedPreferences(context.get()).getString("W_CTE_RUTAHH", ""), id );
    ...
    protected void onPostExecute(ArrayList<JsonArray> mensajes) {
        ...
        if(xceptionFlag){
            ...
            try {
                ...
                long modifico = mDb.update("CensoEquipoFrio", updateValues, "id = ?",new String[]{id});
